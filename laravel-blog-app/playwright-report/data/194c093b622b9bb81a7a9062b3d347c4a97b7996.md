...
- Name: Navigation.spec.ts >> Smoke: home page and navigation >> Products link navigates to the Products section
- Location: tests\Navigation.spec.ts:31:7

# Error details

```
Error: locator.click: Error: strict mode violation: getByRole('link', { name: /^(Products|Produkte|Продукты)$/ }) resolved to 2 elements:
    1) <a href="https://laravelblogapplication.vercel.app/en/products">Products</a> aka getByRole('navigation').getByRole('link', { name: 'Products' })
    2) <a class="btn" href="https://laravelblogapplication.vercel.app/en/products">Products</a> aka getByRole('main').getByRole('link', { name: 'Products' })

Call log:
  - waiting for getByRole('link', { name: /^(Products|Produkte|Продукты)$/ })
